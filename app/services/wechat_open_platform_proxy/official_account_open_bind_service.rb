module WechatOpenPlatformProxy
  class OfficialAccountOpenBindService
    ...
    def create
      ...
      JSON.load(resp.body)["open_appid"].presence&.then do |open_app_id|
        official_account.third_party_platform.open_accounts.create(app_id: open_app_id, principal_name: official_account.principal_name).tap { |open_account| bind(open_account) }
      end
    end

    def bind(open_app_id)
      ...
      if JSON.load(resp.body).errcode.zero?
        official_account.third_party_platform.open_accounts.find_or_create_by(app_id: open_app_id, principal_name: official_account.principal_name).tap { |open_account| bind(open_account) }
      else
        false
      end
    end
    ...
    def unbind(open_app_id)
      ...
      if JSON.load(resp.body).errcode.zero?
        official_account.third_party_platform.open_accounts.find_or_initialize_by(app_id: open_app_id, principal_name: official_account.principal_name).tap { |open_account| bind(nil) }
      else
        false
      end
    end
